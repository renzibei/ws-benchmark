...
//                }
    ...
    int TestWsServer(int argc, char** argv) {
        ...
        if (argc < 6) {
            printf("Invalid parameters!\nUsage: ./echo_client ip_addr port max_msg_len msg_cnt output_data_filename\n");
            return -1;
        }
        ...
        long long msg_cnt = atoll(argv[4]);
        if (msg_cnt <= 0) {
            printf("invalid msg_cnt: %s\n", argv[4]);
            return -1;
        }
        TEST_TIMES = size_t(msg_cnt);

        const char* data_file_path = argv[5];

        FWS_ASSERT(fws::InitEnv(argc - 5, argv + 5) >= 0);

        printf("Set host: %s, port: %d, msg_size: %zu, msg_cnt: %zu, data file path: %s\n",
               SERVER_IP, SERVER_PORT, MAX_DATA_LEN, TEST_TIMES, data_file_path);
